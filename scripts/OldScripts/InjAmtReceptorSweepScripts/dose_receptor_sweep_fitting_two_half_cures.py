...
def overlay_A_half_dots(ax, pivot, beta):
    """
    For each R_C row, fit a sigmoid to cure rate vs A and extract A_half.
    Plot as yellow dots with black outline on the heatmap axes.
    Reports sigma (the row-wise beta equivalent) to terminal.
    """
    dose_cols = np.array(sorted(pivot.columns))  # nmol, ascending
    recep_rows = np.array(pivot.index)  # mol/cell, descending

    def logistic(x, A_half, k):
        return 1.0 / (1.0 + np.exp(-k * (x - A_half)))

    A_halfs = []
    valid_receps = []

    for recep in recep_rows:
        row = pivot.loc[recep, sorted(pivot.columns)].values  # cure rates vs dose

        if row.max() <= 0.0 or row.min() >= 1.0:
            continue
        if row.max() - row.min() < 0.1:
            continue

        try:
            A_mid = dose_cols[len(dose_cols) // 2]
            A_range = dose_cols.max() - dose_cols.min()
            k_init = 4.0 / A_range
            p0 = [A_mid, k_init]
            bounds = ([dose_cols.min(), 0], [dose_cols.max(), np.inf])
            popt, _ = curve_fit(
                logistic, dose_cols, row, p0=p0, bounds=bounds, maxfev=5000
            )
            A_half_fit = popt[0]

            if dose_cols.min() < A_half_fit < dose_cols.max():
                A_halfs.append(A_half_fit)
                valid_receps.append(recep)
        except RuntimeError:
            pass

    if len(valid_receps) < 2:
        print(
            "Warning: fewer than 2 rows cross cure_rate=0.5; row-wise fit unreliable."
        )
        return

    A_arr = np.array(A_halfs)
    RC_arr = np.array(valid_receps)

    # Fit A = sigma * (1 + K_M/A) ... wait, that's R_C(A).
    # Row-wise: A_half = K_M / (R_C/sigma - 1)  =>  R_C = sigma*(1 + K_M/A_half)
    # Same closed-form as fit_boundary but with roles swapped for reporting.
    x = 1.0 + K_M_nmol / A_arr
    sigma = np.dot(RC_arr, x) / np.dot(x, x)

    print("\nRow-wise boundary fit (A_half vs R_C):")
    print(f"  Rows crossing 0.5: {len(valid_receps)} / {len(recep_rows)}")
    print(f"  sigma = {sigma:.4e} mol/cell")
    print("  (compare beta = from column-wise fit above)")

    # Map to heatmap pixel coordinates
    x_pixels = np.interp(A_arr, dose_cols, np.arange(len(dose_cols)) + 0.5)

    recep_asc = recep_rows[::-1]
    row_idx_asc = np.arange(len(recep_rows))[::-1]
    y_pixels = np.interp(
        RC_arr, recep_asc, row_idx_asc + 0.5, left=np.nan, right=np.nan
    )

    mask = np.isfinite(y_pixels)
    ax.scatter(
        x_pixels[mask],
        y_pixels[mask],
        s=7,
...
def create_heatmap(df, output_path):
    ...
    if beta is not None:
        ...
        overlay_A_half_dots(ax, pivot, beta)
