#!/usr/bin/env python3
"""
Study the effect of binding affinity on cumulated activity

Vary k_on (binding rate) to explore affinity landscape.
Higher k_on = higher affinity (stronger binding)

Cumulated activity metric: lambda_decay * Integral of captive radioligand
(N_b^H + N_ic^H) over 30 days, converted to GBq·s.
...
def solve_pk_model(k_on_value):
    ...
    Returns:
        ...
        CumAct: Cumulated activity (GBq·s)
    """
    ...
    # Cumulated activity: lambda_decay [/min] * integral [nmol·min] * [mol/nmol] * N_A [/mol] / 1e9 [/GBq]
    CumAct = lambda_decay * trapezoid(N_captive_H, t_min) * 1e-9 * AVOGADRO / 1e9  # GBq·s

    return N_b_H, N_ic_H, CumAct

# =============================================================================
# SWEEP OVER k_on VALUES
# =============================================================================

print("\n" + "="*70)
print("AFFINITY STUDY: k_on vs Cumulated Activity")
print("="*70)

k_on_min = 0.0000001  # m³/(nmol·min) (low affinity)
k_on_max = 0.1        # m³/(nmol·min) (high affinity)
...
k_on_values = np.sort(np.append(k_on_values, 0.0000015))

print(f"\nScanning {len(k_on_values)} k_on values from {k_on_min} to {k_on_max:.2f} m³/(nmol·min)")
print(f"k_off = {k_off:.2e} /min")
...
for i, k_on in enumerate(k_on_values):
    N_b_H, N_ic_H, CumAct = solve_pk_model(k_on)
    CumAct_values.append(CumAct)
    K_d_values.append(k_off / k_on)

    print(f"  k_on = {k_on:.2e} m³/(nmol·min), K_d = {K_d_values[-1]:.2e} nmol/m³, "
          f"Cumulated Activity = {CumAct:.4f} GBq·s")
...
print(f"\nMaximum cumulated activity: {CumAct_values.max():.4f} GBq·s")
print(f"  Achieved at k_on = {k_on_values[np.argmax(CumAct_values)]:.2e} m³/(nmol·min)")
print(f"  (K_d = {K_d_values[np.argmax(CumAct_values)]:.2e} nmol/m³)")

print(f"\n95% of maximum cumulated activity achieved at:")
print(f"  k_on = {k_on_95:.2e} m³/(nmol·min)")
...
ax1 = axes
ax1.semilogx(k_on_values, CumAct_values, 'o-', linewidth=2, markersize=6)
ax1.axvline(0.0000015, color='orange', linestyle='--', linewidth=1.5, alpha=0.7)
ax1.set_xlabel('$k_{on}$ (binding rate, m$^3$/(nmol·min))')
ax1.set_ylabel('Cumulated Activity (GBq·s)')
ax1.set_title('On rate vs Cumulated Activity', fontweight='bold')
...
output_file = os.path.join(output_dir, 'kon_vs_CumAct.png')
...
for i, (k_on, label, color) in enumerate([
    ...
    ax.plot(t_days, N_ic_H,   label='Internalized ($N_{ic}^H$)',    linewidth=2,   color=color, linestyle=':')
    ax.plot(t_days, N_captive,label='Captive ($N_b^H + N_{ic}^H$)', linewidth=2.5, color=color)

    ax.set_xlabel('Time (days)', fontsize=11)
    ax.set_ylabel('Amount (nmol)', fontsize=11)
    ax.set_title(f'{label}\n$k_{{on}}$ = {k_on:.2e} m³/(nmol·min), '
                 f'$K_d$ = {k_off/k_on:.2e} nmol/m³, Cumulated activity = {CumAct:.4f} GBq·s',
    ...
    print(f"  Cumulated activity = {CumAct:.4f} GBq·s")
