#!/usr/bin/env python3
"""
Create comparison grid of tumor images across parameter sweep

This creates a PDF with images arranged in a grid:
- Rows: Different parameter values (intervals)
- Columns: Time progression (days)

Usage:
    python create_image_grid.py results/debug_sweeps/SWEEP_DIR/
    python create_image_grid.py results/debug_sweeps/SWEEP_DIR/ --days 5 20 40 60
    python create_image_grid.py results/debug_sweeps/SWEEP_DIR/ --replicate 1
"""
...
def find_sweep_directories(sweep_dir):
    """Find all interval_X_rep_Y directories"""
    sweep_path = Path(sweep_dir)
    
    # Pattern: interval_XX_rep_Y
    pattern = re.compile(r'interval_(\d+)_rep_(\d+)')
    ...
    for item in sweep_path.iterdir():
        if item.is_dir():
            ...
            if match:
                ...
                if interval not in dirs:
                    dirs[interval] = {}
                dirs[interval][rep] = item
...
def create_grid_pdf(sweep_dir, output_file, days=None, replicate=None):
    ...
    dirs = find_sweep_directories(sweep_dir)
    
    if not dirs:
        print(f"ERROR: No simulation directories found in {sweep_dir}")
        sys.exit(1)
    
    # Get intervals and replicates
    intervals = sorted(dirs.keys())
    all_reps = set()
    for interval_dict in dirs.values():
        all_reps.update(interval_dict.keys())
    ...
    for rep in replicates:
        ...
        for interval in intervals:
            if interval not in dirs or rep not in dirs[interval]:
                complete = False
                break
        if complete:
    ...
    if days is None:
        ...
        first_dir = dirs[first_interval][first_rep]
    ...
    with PdfPages(output_file) as pdf:
        ...
        for rep in replicates:
            ...
            fig.suptitle(f'Parameter Sweep - Skew=0, Replicate {rep}', 
            ...
            for i, interval in enumerate(intervals):
                ...
                if interval not in dirs or rep not in dirs[interval]:
                    print(f"  WARNING: Missing data for interval={interval}, rep={rep}")
                    continue
                
                sim_dir = dirs[interval][rep]
                ...
                for j, day in enumerate(days):
                    ...
                    # Row labels (intervals) - first column only
                    if j == 0:
                        color = 'green' if outcome == 'CURE' else 'red'
                        ax.text(-0.12, 0.5, f'Interval={interval}\n{outcome}',
...
def create_outcome_summary(sweep_dir, output_file):
    ...
    with open(output_file, 'w') as f:
        ...
        for interval in intervals:
            ...
            outcomes = []
            for rep in replicates:
                if interval in dirs and rep in dirs[interval]:
                    outcome = get_outcome(dirs[interval][rep])
                    f.write(f"{outcome:<9}")
...
def main():
    parser = argparse.ArgumentParser(
        description='Create image comparison grid from debug sweep',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Create grid with all replicates, auto-detected days
  python create_image_grid.py results/debug_sweeps/interval_row_skew0_20260129/
  
  # Specify specific days
  python create_image_grid.py results/debug_sweeps/interval_row_skew0_20260129/ --days 5 20 40 60
  
  # Only show replicate 1
  python create_image_grid.py results/debug_sweeps/interval_row_skew0_20260129/ --replicate 1
        """
    )
